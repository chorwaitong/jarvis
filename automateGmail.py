    ...
    for i in range(0, len(dfUnread)):
        ...
        if stat_code == 0:
            ...
            if outputJSON['urgent'].lower() == 'yes':
                ...
                # Create a draft reply and assign the "urgent" label
                draft_stat_code, draft_msg = skill.gmailCreateDraft(
                    lastUnread.threadID, 
                    lastUnread.sendto, 
                    lastUnread.sendcc, 
                    outputJSON['draft_reply'], 
                    lastUnread.bodyParts
                )
                print('[Code: {}] - {}'.format(draft_stat_code, draft_msg))
            ...
            # If the email requires action
                ...
                print('Drafting Reply and assigning action label')
                
                # Create a draft reply and assign the "action needed" label
                draft_stat_code, draft_msg = skill.gmailCreateDraft(
                    lastUnread.threadID, 
                    lastUnread.sendto, 
                    lastUnread.sendcc, 
